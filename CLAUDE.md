...
- `generate <schema> [-o -f --seed --locale --volume --entity --sql-dialect --sql-indexes --pretty --dry-run --corpus-dir --llm-live]` — formats `json | csv | ndjson | yaml | sql`. `--sql-indexes` is `manual` (default, declared `_indexes` only) | `auto` (also emit inferred indexes) | `none`.
